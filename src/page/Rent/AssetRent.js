...
export default function AssetListPage() {
          ...
          {data.map((item) => (
            <div key={item.barcode} className="card">
              ...
              {item.startDate && item.endDate && (
                <div className="card-row"><span className="card-label">대여기간</span><span className="card-value">{item.startDate} ~ {item.endDate}</span></div>
              )}
            </div>
          ))}
        </div>
  ...
  return (
    <div className="page-container">
      ...
      {isMobile ? renderRentCardList(대여신청내역, '대여 신청 내역', checkedItems, handleCheck) : renderTable(대여신청내역, '대여 신청 내역')}
      {isMobile ? renderRentCardList(내가신청한자산, '내가 신청한 자산', checkedItems, handleCheck) : renderTable(내가신청한자산, '내가 신청한 자산')}
      {isMobile ? renderRentCardList(대여중자산, '대여중', checkedItems, handleCheck) : renderTable(대여중자산, '대여중')}
